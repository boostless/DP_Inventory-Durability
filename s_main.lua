...
ESX.RegisterServerCallback("DP_Inventory:getPlayerInventory", function(source, cb, target)
	...
	local itemDura = {}
	if targetXPlayer ~= nil then
		local result =  MySQL.Sync.fetchAll('SELECT * FROM inventory_durability WHERE owner = @owner', {
			['@owner'] = targetXPlayer.identifier
		})
		if result[1] ~= nil then
			for i=1, #result do
				table.insert(itemDura, {
		...
		MySQL.Async.fetchAll('SELECT * FROM inventory_hotbar WHERE owner = @owner', {
			...
			cb({inventory = targetXPlayer.inventory, money = targetXPlayer.getMoney(), accounts = targetXPlayer.accounts, weapons = targetXPlayer.loadout, slotes = itemSlotes, durability = itemDura})
		end)
	else
		cb(nil)
	end
