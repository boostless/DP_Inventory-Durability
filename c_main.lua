	...
	if not IsPlayerDead(PlayerId()) and canFire then
		if fastWeapons[1] ~= nil then
			if string.find(fastWeapons[1], 'WEAPON_') then
				ESX.TriggerServerCallback('DP_Inventory:checkDurability', function(usable) 
					if not usable then
						ESX.ShowNotification('Item is damaged')
						return
					end
					TriggerServerEvent('esx:useItem', fastWeapons[1])
					TriggerEvent('DP_Inventory:notification', fastWeapons[1], _U('item_used'), 1, false)
				...
				for key, value in pairs(inventory) do
					if inventory[key].count <= 0 then
						...
						for slot, item in pairs(fastWeapons) do
							if item == inventory[key].name then
								table.insert(fastItems, {
									...
									canRemove = true,
									slot = slot,
									durability = getDurability(inventory[key].name, duras)
								})
								founditem = true
								break
							end
						...
						if founditem == false then
							table.insert(inventory[key], durability)
							inventory[key].durability = getDurability(inventory[key].name, duras)
